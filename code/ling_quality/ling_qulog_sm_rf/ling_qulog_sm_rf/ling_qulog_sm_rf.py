...
class LingQulogSmRf:
    ...
    def _get_word_class_embeddings(self, log_lines):
        ...
        word_classses_dep = [" ".join([t.dep_ + "_" + t.text for t in doc]) for doc in docs]
        result_dep = []
        for sent in word_classses_dep:
            if EMPTY_STRING in sent:
                result_dep.append({"obj": 0, "subj": 0, "root": 0})
            else:
                if "obj" in str(sent).lower():
                    obj = 1
                else:
                    obj = 0
                if "subj" in str(sent).lower():
                    subj = 1
                else:
                    subj = 0
                if "root" in str(sent).lower():
                    root = 1
                else:
                    root = 0
                result_dep.append({"obj": obj, "subj": subj, "root": root})
        word_classs_trf_docs = list(self.nlp.pipe(word_classses))
        word_class_embeddings = self._get_sm_embeddings(word_classs_trf_docs)
        return word_class_embeddings, result_dep
    ...
    def predict_batch(self, log_lines: List[str]):
        ...
        word_class_embeddings, result_dep = self._get_word_class_embeddings(log_lines)
        predictions = self.model.predict(word_class_embeddings)
        for i in range(len(result_dep)):
            result_dep[i]["prediction"] = predictions[i]
        return result_dep
