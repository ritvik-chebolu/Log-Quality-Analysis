...
class QulogAttentionNnType1:
    ...
    def load(self, model_file_path):
        '''TODO Implement'''
        return torch.load(model_file_path).cuda()
    ...
    def _create_test_data_loaders(self, load_test, pad_len, batch_size):
        test_data = TensorDataset(torch.tensor(self._get_padded_data(load_test, pad_len=pad_len), dtype=torch.int32))
    ...
    def _predict(self, dataloader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                load = batch[0]
                out = self.model.forward(load.cuda().long())
                tmp = out.cpu().numpy()
    ...
    def predict_batch(self, log_lines: List[str]):
        ...
        for lm in log_lines_dep:
            self._predict_top_N_suitable_tokens(self._mapping_fcn(lm, inverse_data_map_))
        # self.log_lines
        for x in self.log_lines:
            tokenized.append(self.tokenizer.tokenize_test(' '.join(regex.sub('', x).strip().split())))
        print(tokenized)
        ...
        self.results = [{"tags": tag, "prediction":sc, "suggestions":sug} for tag, sc, sug in zip(self.log_lines, scores, self.predictions)]
    ...
    def _predict_top_N_suitable_tokens(self, bad_data, N=100):
        ...
        for query in output.index:
            predictions.append((self._recommendation_rules(list(output.columns[np.argsort(output.loc[query])]), query=query, N=10), query))
